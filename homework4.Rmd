...
test_total <- bechdel %>% count(test) %>%
  mutate(prop = n/sum(sum(n))*100)
...
glimpse(bechdel_train)
glimpse(bechdel_test)

total_train <- nrow(bechdel_train)
total_test <- nrow(bechdel_test)
prop_train <- round(total_train/(sum(total_train,total_test)), 4)
prop_test <- round(1 - prop_train, 4)

cat(total_train, total_test, prop_train, prop_test)
...
## Some of the graphs that stand out from the explanatory data analysis below are:
## - budget_2013 vs. test: the median budget is lower for movies that pass the test.
## - domgross_2013 and intgross_2013 vs. test: there are more outliers in terms of higher domestic and international revenues for movies that fail the test.
## - imbdb_rating vs. test: the distribution of imdb ratings is more negatively skewed for movies that pass the test (they tend to have a lower score).
## - metascore vs. test: the same is for metascore, although the skewedness fo the distribution is not as noticeable.
...
## The first code summarizes the data by movie genre, indicating how many (in number and proportion) of any given genre's movies passed/failed the test.
## The second code summarizes the data by movie rating, indicating how many (again in muber and proportion) of any given rating's movies (PG, PG13, etc) passed/failed the test. It is interesting that for NC17 ratings, the proportion of movies that failed the test is the highest.
...
## It returns the 10 folds that the vfold_cv function splits the training data into. These are all of almost the same size.
```
...
```{r}
movie_rec <- recipe(test ~ budget_2013 + intgross_2013 + rated + metascore + imdb_rating + genre, data = bechdel) %>%
  step_other(genre, threshold = .03) %>% 
  step_novel(all_nominal(), -all_outcomes()) %>% # Use *before* `step_dummy()` so new level is dummified
  step_dummy(all_nominal(), -all_outcomes()) %>% 
  step_zv(all_numeric(), -all_outcomes())  %>% 
...
## PLEASE NOTE: I removed the predictor 'domgross_2013' because when running with all predcitors the regressions would not converge.
