...
memberpayments_db <- left_join(totalpayments_db,
    members_db %>% rename(member_id = id),
    by = "member_id") %>%
  arrange(desc(total))

glimpse(memberpayments_db)

# Theresa May received the most amount of money ($2,809,765.4).
...
totalentitypmts_db <-  partydonations_db %>% 
  group_by(entity) %>%
  summarise(
    total = sum(value),
    n = n()) %>%
...
glimpse(totalentitypmts_db)

# Unite and Lord David Sainsbury account for 6.9% and 6.3% of all donations, respectively.
...
totalentitys_db <-  partydonations_db %>% 
  group_by(entity) %>%
  summarise(n = n()) %>%
  mutate(count = 1, total = sum(count))

glimpse(totalentitys_db)

# There are 1,077 distinct entities that paid money to MPs. I did not understand how to find single party donations to answer the second question.
...
glimpse(parties_db)
glimpse(partydonations_db)

totalpartydonations_db <- left_join(partydonations_db,
    parties_db %>% rename(party_id = id),
    by = "party_id") %>%
  mutate(year = substr(as.character(date), 0, 4))

glimpse(totalpartydonations_db)

partysummary_db <-  totalpartydonations_db %>% 
  group_by(year, name) %>%
  summarise(
    total = sum(value),
    n = n()) %>%
...
ggplot(partysummary_db, aes(year, total, fill = name, color = name)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(name = "Donations", labels = comma) +
  labs(title = "Donations to political parties, 2020-2022")
...
tic()
female_icudata <- cdc_data %>%
  group_by(age_group) %>%
  filter(sex =="Female", icu_yn == "Yes", death_yn == "Yes") %>%
  summarize(female_cfd = n()) %>%
  arrange(age_group) %>%
  collect()
toc()

tic()
female_noicudata <- cdc_data %>%
  group_by(age_group) %>%
  filter(sex =="Female", icu_yn == "No", death_yn == "Yes") %>%
  summarize(female_noicu = n()) %>%
  arrange(age_group) %>%
  collect()
toc()

tic()
male_icudata <- cdc_data %>%
  group_by(age_group) %>%
  filter(sex =="Male", icu_yn == "Yes", death_yn == "Yes") %>%
  summarize(male_icu = n()) %>%
  arrange(age_group) %>%
  collect()
toc()

tic()
male_noicudata <- cdc_data %>%
  group_by(age_group) %>%
  filter(sex =="Male", icu_yn == "No", death_yn == "Yes") %>%
  summarize(male_noicu = n()) %>%
  arrange(age_group) %>%
  collect()
toc()

total_data <- list(female_icudata, female_noicudata, male_icudata, male_noicudata) %>%
  reduce(full_join, by='age_group')

glimpse(total_data)

years_18to49 <- t(total_data[1,2:5])
years_50to64 <- t(total_data[2,2:5])
years_65plus <- t(total_data[3,2:5])

final_data <- data.frame(
        year_group = rep(c("years_0to17", "years_18to49","years_50to64","years_65plus"), each = 4, times = 1),
        value = c(c(0,0,0,0), years_18to49, years_50to64, years_65plus),
        category = rep(c("female/icu", "female/no icu", "male/icu", "male/no icu"), each = 1, times = 4)
        )

ggplot(final_data) +
  geom_bar(aes(x = value, y = year_group, fill = category), position = "stack", stat = "identity") + 
  facet_wrap(~category)
...
tic()
female_icudata2 <- cdc_data %>%
  group_by(case_month) %>%
  filter(sex =="Female", icu_yn == "Yes", death_yn =="Yes") %>%
  summarize(female_icu = n()) %>%
  arrange(case_month) %>%
  collect()
toc()

tic()
female_noicudata2 <- cdc_data %>%
  group_by(case_month) %>%
  filter(sex =="Female", icu_yn == "No", death_yn =="Yes") %>%
  summarize(female_noicu = n()) %>%
  arrange(case_month) %>%
...
total_data2 <- list(female_icudata2, female_noicudata2, male_icudata2, male_noicudata2) %>%
  reduce(full_join, by='case_month')

glimpse(total_data2)

# I reproduced a table that summarizes the data needed to recreate the graph but could not append the columns as in the previous question to a single one.
...
tic()
death_data <- cdc_data %>%
  group_by(county_fips_code, case_month) %>%
  filter(death_yn == "Yes") %>%
  summarize(death = n()) %>%
  arrange(case_month) %>%
  collect()
...
urbanrural_dp <- urban_rural %>% rename(county_fips_code = fips_code)

total_data3 <- list(death_data, urbanrural_dp) %>%
  reduce(full_join, by='county_fips_code')

red_data3 <- subset(total_data3, select = c(county_fips_code, case_month, death, county_2012_pop, x2013_code))
red_data3$county_2012_pop <- as.numeric(red_data3$county_2012_pop)

totalCFR <-  red_data3 %>% 
  group_by(case_month, x2013_code) %>%
  summarise(
    totaldeath = sum(death),
    totalpop = sum(county_2012_pop)) %>%
  mutate(prop = totaldeath/totalpop*100) %>%
  arrange(desc(case_month))

glimpse(totalCFR)

# As in the question above, I was able to create a table with the data needed to reproduce the graphs but could not append to a single column.
...
urbanCFR <-  totalCFR %>% 
  ...
  group_by(case_month) %>%
  summarise(
    totaldeath_urban = sum(totaldeath),
    totalpop_urban = sum(totalpop)) %>%
  mutate(prop_urban = totaldeath_urban/totalpop_urban*100) %>%
  arrange(desc(case_month))
...
ruralCFR <-  totalCFR %>% 
  ...
  group_by(case_month) %>%
  summarise(
    totaldeath_rural = sum(totaldeath),
    totalpop_rural = sum(totalpop)) %>%
  mutate(prop_rural = totaldeath_rural/totalpop_rural*100) %>%
  arrange(desc(case_month))

total_data4 <- list(urbanCFR, ruralCFR) %>%
  reduce(full_join, by='case_month')
...
df2022 <- scrape_pac("https://www.opensecrets.org/political-action-committees-pacs/foreign-connected-pacs/2022")

df2020 <- scrape_pac("https://www.opensecrets.org/political-action-committees-pacs/foreign-connected-pacs/2020")

df2000 <- scrape_pac("https://www.opensecrets.org/political-action-committees-pacs/foreign-connected-pacs/2000")
...
for (i in 1:23)
{
    x <- paste("https://www.opensecrets.org/political-action-committees-pacs/foreign-connected-pacs/", n, sep = "")
    y <- append(y, x)
...
scrape_pac2 <- function(x){
    
    for (i in 1:23)
    {
    base_url <- x[i]
    base_year <- str_sub(x[i],85,89)
...
contributions_all <- scrape_pac2(urls)
